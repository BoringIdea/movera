        ...
        <section className="mv-panel space-y-4 px-6 py-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <div className="space-y-1">
              ...
              <a href={`${getExplorerUrl(chain)}/object/${schema.address}`} className="font-mono break-all text-black transition-colors hover:text-[#5f9bff]">
                {schema.address}
              </a>
            </div>
            <div className="space-y-1">
              <p className="mv-kicker">Creator</p>
              <Link href={`/address/${schema.creator}`} className="font-mono break-all text-black transition-colors hover:text-[#5f9bff]">
                {schema.creator}
              </Link>
            </div>
            <div className="space-y-1">
              <p className="mv-kicker">Transaction</p>
              <a href={`${getExplorerTxUrl(chain)}/${schema.tx_hash}`} className="font-mono break-all text-black transition-colors hover:text-[#5f9bff]">
                {schema.tx_hash}
              </a>
            </div>
            <div className="space-y-1">
              <p className="mv-kicker">Resolver</p>
              <p className="text-base text-black">{schema.resolver ? 'Yes' : 'None'}</p>
            </div>
          </div>
        </section>
